...
/**
 * 创建了一个自定义帧布局，把baseFragment一部分代码抽取到这个类中
 * 作者：ZYJ
 * 时间：2015/8/3 0003 14:48
 */
public abstract class LoadingPage extends FrameLayout {
     ...
     * @return
     */
    protected abstract LoadResut load();
    ...
    public void show() {
        ...
        ThreadManager.getInstance().createLongPool().execute(new Runnable() {
            ...
            public void run() {
                ...
                final LoadResut resut = load();

                UiUtils.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (resut != null) {
                            state = resut.getValue();
                            showPage();//状态改变了，重新判断应该显示哪一个界面
                        }
                    }
                });
            }
